    ...
    return (<div >
        ...
        {props.users.map(user => <div key={user.id} className={s.item}>
            <span>
                ...
                <div>
                    {user.followed ?
                        <button disabled={props.followingFetching.some(id => id === user.id)} onClick={() => { props.unfollow(user.id) }}>Unfollow</button> :
                        <button disabled={props.followingFetching.some(id => id === user.id)} onClick={() => { props.follow(user.id) }}>Follow</button>}
